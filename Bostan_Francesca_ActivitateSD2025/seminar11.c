...
void pushStack(Nod** stiva, Masina masina) { //inserare la inceput
...
Masina popStack(Nod** stiva) { //extragere de la inceput
	if (!(*stiva)) {
		return (Masina) { -1, 0, 0, NULL, '-' };
	}
	Nod* temp = *stiva;
	Masina m = temp->info;
	*stiva = temp->next;
